...
$sql = "DELETE FROM `announcements` WHERE Created_On < NOW() - INTERVAL 1 DAY AND Room_ID = ?";
...
if (isset($_POST['announce'])) {
    ...
    $sql = "INSERT INTO `announcements` (Room_ID, User_ID, Title, Description) VALUES (?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssss", $room_id, $user_id, $title, $description);

    if ($stmt->execute()) {
        echo '<script>window.alert("Announcement Added Successfully!");</script>';
    } else {
...
$sql = "SELECT a.ID, a.Title, a.Description, a.User_ID, u.Name FROM announcements a LEFT JOIN users u ON a.User_ID = u.ID WHERE Room_ID = ? ORDER BY a.ID DESC";
$stmt = $conn->prepare($sql);
$stmt->bind_param("s", $_SESSION['room_id']);
$stmt->execute();
...
if (isset($_GET['view_announcement'])) {
    ...
    $query = $conn->prepare("SELECT a.ID, a.Title, a.Description, a.User_ID, u.Name FROM announcements a LEFT JOIN users u ON a.User_ID = u.ID WHERE a.ID = ? ORDER BY a.ID DESC");
...
if (isset($_POST['submitTimeline'])) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Timeline entry added successfully.'); window.location.href ='index.php?room=" . $_SESSION['room_code'] . "';</script>";
        exit();
    } else {
