...
while ($row = mysqli_fetch_assoc($query)) {
    $sql2 = "SELECT * FROM `messages` WHERE (Incomming_ID = ? OR Outgoing_ID = ?) AND (Outgoing_ID = ? OR Incomming_ID = ?) ORDER BY ID DESC LIMIT 1";
    $stmt2 = $conn->prepare($sql2);
    if ($stmt2 === false) {
        die("Error preparing statement: " . $conn->error);
    }
    $stmt2->bind_param("ssss", $row['User_ID'], $row['User_ID'], $user_id, $user_id);
    $stmt2->execute();
